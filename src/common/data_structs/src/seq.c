...
#include "data_structs/arrayrep.h"

#include "data_structs/seq.h"
...
void*
Seq_put(Seq_T seq, unsigned i, void* x)
{
  void* prev;
...
Seq_addlo(Seq_T seq, void* x)
{
  ...
  return ((void**)seq->storage.mem_alloc)
         [(seq->head + i) % seq->storage.length] = x;
}
