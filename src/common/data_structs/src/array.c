...
void
ArrayRep_init(struct array_rep* p_arrayRep, size_t length, size_t size,
              void* mem_alloc)
{
  Assert(p_arrayRep);
...
void*
Array_get(Array_T self, unsigned i)
{
  Assert(self);
...
void*
Array_put(Array_T self, unsigned i, void* elem)
{
  Assert(self);
  Assert(i < self->length);
  Assert(elem);
...
void
Array_resize(Array_T self, size_t length)
{
  Assert(self);
...
Array_T
Array_copy(Array_T self, size_t length)
{
  Array_T arrayT;
