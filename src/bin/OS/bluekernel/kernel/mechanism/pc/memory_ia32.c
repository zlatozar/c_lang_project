...
int
AddMemoryMec(process_table_t* pID_ptr)
{
...
// @P+
int
AddPageMec(process_table_t* pID_ptr, int32_t physical_page, uint32_t virtual_address,
           uint32_t permissions)
{
  int32_t   page_table;
...
int32_t
RemovePageMec(process_table_t* pID_ptr, uint32_t virtual_address)
{
...
// @T+
void
EditPageMec(uint32_t physical_page)
{
  uint32_t* scratchpad;
...
uint32_t
GetMemoryMec(process_table_t* pID_ptr, int property, uint32_t virtual_address)
{
  if (pID_ptr != kernel_ptr)
  { EditPageMec(pID_ptr->base_page); }
  else
...
uint32_t
GetPhysAddrMec(process_table_t* pID_ptr, uint32_t virtual_address)
{
  if (pID_ptr != kernel_ptr)
  { EditPageMec(pID_ptr->base_page); }
  else
...
// @P+
uint32_t
GetPermissionsMec(process_table_t* pID_ptr, uint32_t virtual_address)
{
  if (pID_ptr != kernel_ptr)
  { EditPageMec(pID_ptr->base_page); }
  else
...
*/
uint32_t
AddPageTableEntryMec(uint32_t physical_page, uint32_t index, uint32_t permissions)
{
...
int32_t
GetPageTableMec(uint32_t virtual_address)
{
  uint32_t* scratchpad;
