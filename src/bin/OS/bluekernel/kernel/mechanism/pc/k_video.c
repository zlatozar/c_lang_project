...
void
BgaWriteRegister(uint16_t index, uint16_t value)
{
...
uint16_t
BgaReadRegister(uint16_t index)
{
  Out16(BGA_INDEX, index);
...
void
VideoExceptionOut(uint32_t exc_msg)
{
  uint32_t*  stack_addr;
  int     show_esp_ss = 1;
...
void
VideoAddPage(int32_t curr_pID, int32_t pID, uint32_t virtual_address, uint32_t getphys)
{
  video_mode = V_EXCEPTION;
...
int
VideoProcessOut(struct process_table_t* pID_ptr)
{
...
int
VideoStrNumOut(const char* s, uint32_t num, int width)
{
  VideoStrOut(s);
...
int
VideoNumOut(uint32_t num, int width)
{
  char c[8];
  int i, temp;
  char* video_temp;
