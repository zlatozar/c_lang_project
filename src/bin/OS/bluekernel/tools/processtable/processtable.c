...
int
ConvertOptions(char* options, char* error_code)
{
  if (options[0] != '-')
  { exit(-4); }
  else {
    ...
    while (options[i] != 0) {
      switch (options[i]) {
        case 'a' :
          option_all = option_d = option_b = option_m = option_p = option_s = option_x = 1;
          break;
        case 'd' :
          option_d = 1;
          break;
...
ConvertTextInfile(char* infile)
{
  ...
  while (!feof(g_file_in_ptr)) {
    fscanf(g_file_in_ptr, "%X%s%s%X%X%X%X\n", &x1, s1, s2, &ptaddr[i], &ptaddr[i + 1], &ptaddr[i + 2],
           &ptaddr[i + 3]);
    i += 4;
  }
...
int
ConvertBinaryInfile(char* infile)
{
...
PrintTable(void)
{
  ...
  while (pt_ptr <= last_pt_ptr) {
    if (pt_ptr->status != 0) {
      ...
      dprint("%04x%c %01x %02x %01x " NORMAL, pt_ptr->pID, privilege, pt_ptr->status,
             pt_ptr->times_scheduled, pt_ptr->priority);
  ...
  dprint(NORMAL "num_processes: %04x (word count for bochs %d) ", first_pt_ptr->msg[2],
         first_pt_ptr->msg[2]*PROCESS_TABLE_ENTRY_SIZE);
...
MessageQueues(void)
{
  ...
  */
  ...
  while (pt_ptr <= last_pt_ptr) {
    ...
    if (pt_ptr != msgQ_ptr
        && msgQ_ptr !=
        0) { // originally message empty message queues were indicated by 0, now by 'self' pointer. This line makes ptd tool compatible with old and new dump files.
      char reason = '?';
...
int32_t
Convert_ptr_pID(process_table_t* pID_ptr)
{
...
process_table_t*
Convert_kptr_ptr(process_table_t* kptr)
{
...
int
PrintReason(process_table_t* pt_ptr)
{
...
int
PrintFamily(process_table_t* pt_ptr)
{
...
int
PrintMechanisms(void)
{
  process_table_t* pt_ptr = first_pt_ptr;
  uint32_t esp0 = 0, eip = 0, eflags = 0, eax = 0, ebx = 0, ecx = 0, edx = 0, esp = 0, ebp = 0,
           esi = 0, edi = 0;
