/* yams -- Yet Another Machine Simulator
   ...
   $Id: elf.c,v 1.4 2016/02/09 08:56:42 jaatroko Exp $
*/
...
int
load_elf(char* fname, uint32_t* entry_point)
{
  FILE* f;
  int ret;
...
static int
load_elf_file(FILE* f, uint32_t* entry_point)
{
  Elf32_Ehdr elfhdr;
  Elf32_Phdr phdr;
  int i;
  ...
  for (i = 0; i < simtoh16(elfhdr.e_phnum); i++) {
    ...
    switch (simtoh32(phdr.p_type)) {
      ...
      case PT_MIPS_ABIFLAGS:
        printf("ELF: ignoring MIPS ABIFLAGS program header\n");
        break;
      case PT_LOAD:
