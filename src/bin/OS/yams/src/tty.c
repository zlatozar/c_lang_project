...
static int
tty_init_inet(char* name, int port, int slisten, ttydevice_t* tty)
{
...
static int
tty_init_unix(char* name, int port, int slisten, ttydevice_t* tty)
{
...
int
tty_init(int domain, char* name, int port, int listen, device_t* tty_dev)
{
  int ret = 0;
  ttydevice_t* tty;
...
int
tty_setdelay(int delay, device_t* tty_dev)
{
  ttydevice_t* tty;
  tty = (ttydevice_t*)tty_dev->realdevice;
...
int
tty_io_write(device_t* dev, uint32_t addr, uint32_t data)
{
...
int
tty_io_read(device_t* dev, uint32_t addr, uint32_t* data)
{
  ttydevice_t* tty = dev->realdevice;
  if (dev == NULL || dev->realdevice == NULL) { return -1; } /* fatal */
...
static void
tty_raise_read_interrupt(ttydevice_t* tty)
{
...
static void
tty_raise_write_interrupt(ttydevice_t* tty)
{
  if (!tty->wirq_pending && !tty->rirq_pending) {
    tty->irq_processor = select_cpu_for_irq();
  }
