...
void
disk_destroy(device_t* disk_dev)
{
 ...
 */
int
disk_init(char* filename, int sectorsize, int numsectors,
          device_t* disk_dev)
{
  diskdevice_t* disk;
  FILE* f;
...
int
disk_setparams(int numcylinders, int time_rot, int time_fullseek,
               device_t* disk_dev)
{
  int retval = 0;
  diskdevice_t* disk;
 ...
 */
static uint64_t
calculate_seek_time(diskdevice_t* disk)
{
...
int
disk_io_write(device_t* dev, uint32_t addr, uint32_t data)
{
  diskdevice_t* disk;
...
int
disk_io_read(device_t* dev, uint32_t addr, uint32_t* data)
{
  diskdevice_t* disk;
  if (dev == NULL || dev->realdevice == NULL) { return -1; } /* fatal */
  if (addr >= IOLENGTH_DISK || (addr & 3))
