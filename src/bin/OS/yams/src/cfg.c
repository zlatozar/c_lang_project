...
int
cfg_simoption_clockspeed(uint32_t speed)
{
  clock_speed = speed;
  return 1;
}
...
int
cfg_simoption_memory(uint32_t memory)
{
  mem_pages = memory;
  return 1;
}
...
int
cfg_simoption_cpus(uint32_t cpus)
{
  num_cpus = cpus;
  return 1;
}
...
int
cfg_simoption_cpuirq(uint32_t irq)
{
  if (irq < 0 || irq > 5) {
    printf("\
...
int
cfg_simoption_endianness(int bigendian)
{
