...
static pluggable_device_t*
lookup_dev(int fd, int tag)
{
...
void
plugio_destroy(device_t* plugio_dev)
{
...
static int
plugio_init_inet(char* name, int port, int slisten,
                 int sock, int* desc)
{
...
static int
plugio_init_unix(char* name, int port, int slisten,
                 int sock, int* desc)
{
 ...
 */
static void
READ(int fd, void* rbuf, size_t count)
{
  ssize_t r;
  char* buf = rbuf;
...
static void
write_wrap(int fd, void* rbuf, size_t count)
{
  ssize_t r;
  char* buf = rbuf;
...
static void
WRITE(int fd, void* rbuf, size_t count, int now)
{
...
int
plugio_init(int domain, char* name, int port, int listen,
...
int
plugio_mmap(pluggable_device_t* plugio, uint32_t addr)
{
  uint32_t cmd;
...
static void
handle_replies(pluggable_device_t* pdev,
...
int
plugio_mmap_read(pluggable_device_t* plugio, uint32_t addr,
                 void* buf, int size)
{
  uint32_t d;
...
/* addr is relative to the start of the area */
int
plugio_mmap_write(pluggable_device_t* plugio, uint32_t addr,
                  void* buf, int size)
{
  uint32_t d;
...
int
plugio_io_write(device_t* dev, uint32_t addr, uint32_t data)
{
...
int
plugio_io_read(device_t* dev, uint32_t addr, uint32_t* data)
{
  uint32_t io_port = htonl(addr >> 2);
  pluggable_device_t* plugio;
  uint32_t cmd;
