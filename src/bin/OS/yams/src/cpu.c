...
uint32_t
cpu_register_to_number(char* name)
{
  int i;
...
static void
raise_exception(cpu_t* cpu, exception_t which)
{
  cpu->exception_pending = which;
}

static void
raise_address_exception(cpu_t* cpu, exception_t which, uint32_t addr)
{
...
static void
raise_cp_exception(cpu_t* cpu, exception_t which, uint32_t cp_nro)
{
  cpu->exception_pending = which;
...
void
disasm_instruction(uint32_t addr,
                   uint32_t instr_word,
                   char* buf,
                   int len)
{
...
static void
invalidate_rmw_sequences(uint32_t vAddr)
{
  int i;
...
int
cpu_exception_condition(cpu_t* cpu, uint32_t* exceptionCode)
{
  uint32_t ex;
