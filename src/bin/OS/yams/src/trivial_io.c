...
/* port 0: cpu status */
int
cpuinfo_io_read(device_t* dev, uint32_t addr, uint32_t* data)
{
  int cpunum;
  cpuinfo_t* cpu;
...
/* port 1: command port */
int
cpuinfo_io_write(device_t* dev, uint32_t addr, uint32_t data)
{
  int cpunum;
  cpuinfo_t* cpu;
...
int
meminfo_io_read(device_t* dev, uint32_t addr, uint32_t* data)
{
 ...
 */
int
rtc_io_read(device_t* dev, uint32_t addr, uint32_t* data)
{
  if (dev == NULL) { return -1; }
...
int
shutdown_io_write(device_t* dev, uint32_t addr, uint32_t data)
{
  if (dev == NULL) { return -1; }
  if (addr != 0) { return 1; }
