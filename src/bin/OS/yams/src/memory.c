...
memory_t*
memory_create(uint32_t pagesize, uint32_t num_pages)
{
  memory_t* memory;
 ...
 */
uint32_t
phys_addr(uint32_t vaddr, cpu_t* cpu)
{
  uint32_t paddr;
...
exception_t
mem_read(memory_t* mem, uint32_t addr, void* buf,
         int size, cpu_t* cpu)
{
  int ioread = 0;
  uint8_t* readfrom;
...
exception_t
mem_write(memory_t* mem, uint32_t addr, void* buf,
          int size, cpu_t* cpu)
{
  int iowrite = 0;
  exception_t exc;
...
int
mem_dread(memory_t* mem, uint32_t addr, void* buf, int size)
{
...
int
mem_dwrite(memory_t* mem, uint32_t addr, void* buf, int size)
{
  assert(size == 1 || size == 2 || size == 4);
...
int
mem_store_direct(memory_t* mem, uint32_t addr,
...
int
mem_read_direct(memory_t* mem, uint32_t addr,
                uint32_t length, void* data)
{
  uint32_t size = mem->num_pages * mem->pagesize;
