...
static int
hexToNum(char** ptr, int* intValue, int length)
{
  int numChars = 0;
  int hexValue;
 ...
 *************************************************/
static uint32_t
read_register(int cpunum, int regnum)
{
...
static void
read_registers(int cpunum, char* result)
{
  int i;
...
static void
write_register(int cpunum, int regnum, uint32_t new_val)
{
  cpu_t* cpu = hardware->cpus[cpunum];
...
static void
write_registers(int cpunum, char* new_values)
{
  uint32_t new_val;
  int i;
...
static int
read_mem(uint32_t addr, uint32_t length, char* result)
{
  exception_t exc;
...
static int
write_mem(uint32_t addr, uint32_t length, char* new_value)
{
  exception_t exc;
  uint32_t save_status;
  int mem_byte;
 ...
 ********************/
void
gdb_interface_open(uint16_t port)
{
 ...
 * continue */
int
gdb_interface_check_and_run(void)
{
