...
int
hwconsole_handle_command(char* cmd)
{
...
#ifndef READLINE
static void
hwconsole_print_prompt(char* prompt)
{
...
static char*
replace_with_gnu_readline(char* prompt)
{
...
void
command_step(uint32_t stepcount)
{
...
void
command_breakpoint(uint32_t address)
{
...
void
command_quit(uint32_t exitcode)
{
  if (exitcode > 255) {
...
*/
int
command_memwrite(uint32_t address, char* filename)
{
  FILE* f;
...
*/
void
command_memread(uint32_t address, uint32_t length, char* filename)
{
  FILE* f;
  char buf[SIMULATOR_PAGESIZE];
  int n;
...
static void
print_cpu_register(cpu_t* cpu, int regnro)
{
  if (regnro >= 32) {
    printf("   %-5s = %.8" PRIx32 " ",
...
static void
print_cp0_register(cpu_t* cpu, int regnro)
{
  if (regnro >= 32) {
    printf("   %-6s= %.8" PRIx32 " ",
...
void
command_regdump(uint32_t processor_id)
{
  int i;
...
void
command_interrupt(uint32_t interrupt_number, uint32_t cpu_number)
{
...
void
command_cpuregwrite(uint32_t processor_number,
...
void
command_cp0regwrite(uint32_t processor_number,
                    uint32_t register_number,
                    uint32_t value)
{
  if (processor_number >= hardware->num_cpus) {
...
void
command_dump_regsource(uint32_t processor_number,
                       uint32_t register_number,
                       uint32_t words)
{
  if (processor_number >= hardware->num_cpus) {
...
void
command_dump(uint32_t address, uint32_t words)
{
  uint32_t w;
...
void
command_poke(uint32_t address, uint32_t word)
{
  exception_t exc;
  uint32_t save_status;
...
void
command_tlbdump(uint32_t processor_id)
{
  uint32_t i;
  cp0_t* cp0;
  ...
  for (i = 0; i < NUM_TLB_ENTRIES; i++) {
    ...
    printf("%2" PRIx32 ": %.8" PRIx32 " %.5" PRIx32 " %" PRId32 " %.4" PRIx32 " %.8" PRIx32 " %.2"
           PRIx32 " %.1" PRIx32 "  %.1" PRIx32 "  %.8" PRIx32 " %.2" PRIx32 " %.1" PRIx32 "  %.1" PRIx32 "\n"
           , i,
           TLB_VPN2(cp0, i) << 13,
           TLB_VPN2(cp0, i),
           TLB_G(cp0, i),
...
void
command_boot(char* kernel_image, char* arguments)
{
  int i;
  int l;
