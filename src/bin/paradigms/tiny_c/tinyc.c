...
node*
sum()  /* <sum> ::= <term> | <sum> "+" <term> | <sum> "-" <term> */
{
  node* t, *x = term();
...
void
g(code c) { *here++ = c; } /* missing overflow check */

code*
hole() { return here++; }

void
fix(code* src, code* dst) { *src = dst - src; } /* missing overflow check */
